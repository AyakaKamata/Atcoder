...
int main(){
  ...
  // x^3 - y^3 = N で x-y は少なくとも 1 となるため、
  // 展開すると (y+1)^3 - y^3 = 3y^2 + 3y + 1 が下限となる。
  // したがって、解が存在するなら N >= 3y^2+3y+1 となり、特に y <= sqrt(N/3) が成り立つ。
  ll bound = (ll)floor(sqrt((long double)N / 3.0)) + 1;

  for(ll y = 1; y <= bound; y++){
      ll y3 = y * y * y;
      ll x3 = y3 + N;
      // x^3 = x3 となる正整数 x を二分探索で求める
      ll lo = 1, hi = 1000000 + 100; // N の最大値が 1e18 なので、x は概ね 1e6 程度
      ll x = -1;
      while(lo <= hi){
          ll mid = (lo + hi) / 2;
          ll mid3 = mid * mid * mid;
