...
int main() {
  ...
  in(n);
  while (n != 0) {
    vector<vector<vector<ll>>> train(24, matrix(60, onevec(60, 0)));
    rep(i, 0, n) {
      string s, t;
      in(s, t);
      // s=hh:mm:ttという文字列
      ...
      int st = stoi(s.substr(6, 2));
      int th = stoi(t.substr(0, 2));
      int tm = stoi(t.substr(3, 2));
      int tt = stoi(t.substr(6, 2));
      train[sh][sm][st]++;
      train[th][tm][tt]--;
    }
    rep(i, 0, 24) rep(j, 0, 60) rep(k, 0, 60) {
      if (k != 0) {
        train[i][j][k] += train[i][j][k - 1];
      } else {
        if (j != 0) {
          train[i][j][k] += train[i][j - 1][59];
        } else {
          if (i != 0) {
            train[i][j][k] += train[i - 1][59][59];
          }
        }
      }
    }
    ll ans = 0;
    rep(i, 0, 24) rep(j, 0, 60) rep(k, 0, 60) { chmax(ans, train[i][j][k]); }
    out(ans);
    in(n);
  }
  return 0;
