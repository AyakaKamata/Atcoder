...
int main() {
  matrix a(3, onevec(3));
  vector a_(3, vector<bool>(3, false));
  rep(i, 0, 3) rep(j, 0, 3) in(a[i][j]);
  ...
  bool f = false;
  rep(i, 0, 3) if (a_[i][0] && a_[i][1] && a_[i][2]) f = true;
  rep(i, 0, 3) if (a_[0][i] && a_[1][i] && a_[2][i]) f = true;
  if (a_[0][0] && a_[1][1] && a_[2][2])
    f = true;
  if (a_[2][0] && a_[1][1] && a_[0][2])
    f = true;
  if (f) {
