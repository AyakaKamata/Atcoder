...
template <typename T> ld twiced(const T &a) { return a * a; }
...
int main() {
  ...
  ld a, b, h, m;
  in(a, b, h, m);
  ld angle1 = 30.0 * h + 0.5 * m;
  ld angle2 = 6.0 * m;
  ld ax = a * cos(angle1 * M_PI / 180.0), ay = a * sin(angle1 * M_PI / 180.0);
  ld bx = b * cos(angle2 * M_PI / 180.0), by = b * sin(angle2 * M_PI / 180.0);
  print(sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by)), 12);
  return 0;
}