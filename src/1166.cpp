...
void bfs_matrix(const matrix &field, ll sx, ll sy, matrix &dist) {
...
int main() {
  ...
  while (!(w == 0 && h == 0)) {
    /* 盤面 */
    matrix field(h * 2 - 1, onevec(w * 2 - 1, 0));
    rep(i, 0, h * 2 - 1) {
      if (i % 2 == 0) {
        rep(j, 0, w - 1) { in(field[i][j * 2 + 1]); }
      } else {
        rep(j, 0, w * 2 - 1) { field[i][j] = 1; }
        rep(j, 0, w) { in(field[i][j * 2]); }
      }
    }
    // rep(i, 0, h * 2 - 1) {
    //   rep(j, 0, w * 2 - 1) { cout << field[i][j]; }
    //   cout << endl;
    // }

    // 各セルの最短距離 (訪れていないところは -1 としておく)
    vector<vector<ll>> dist(h * 2 - 1, onevec(w * 2 - 1, -1));
    ...
    //   cout << endl;
    // }
    if (dist[h * 2 - 2][w * 2 - 2] < 0) {
      out(0);
    } else {
      out(dist[h * 2 - 2][w * 2 - 2] / 2+1);
    }
    in(w, h);
  }
  return 0;
