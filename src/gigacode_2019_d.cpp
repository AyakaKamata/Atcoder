...
int main() {
  ...
  ll h, w, k, v;
  in(h, w, k, v);
  matrix tera(h, onevec(w));
  rep(i, 0, h) rep(j, 0, w) {
    in(tera[i][j]);
    tera[i][j] += k;
  }
  matrix cumsum = CumsumSquare(tera);
  ll ans = 0;
  rep(i, 1, h+1) rep(j, 1, 1+w) rep(k, i, h+1) rep(l, j, w+1) {
    if (cumsum[k][l] - cumsum[i - 1][l] - cumsum[k][j - 1] +
            cumsum[i - 1][j - 1] <=
        v) {
