...
ll rec(ll l, ll r, ll s) {
  ...
  if (l == r)
    return target = (s ? 0 : b[l]);

  if (s) {
    return target = (b[l] > b[r]) ? rec((l + 1) % n, r, 0)
                                  : rec(l, (r + n - 1) % n, 0);
  }

  return target = max(b[l] + rec((l + 1) % n, r, 1),
                      b[r] + rec(l, (r + n - 1) % n, 1));
}
...
int main() {
  ...
  dp.assign(n, onevec(n, -1));
  ll res = 0;
  rep(i, 0, n) { // どれをJOIが抜くか
    chmax(res, rec((i + 1) % n, (i + n - 1) % n, 1) + b[i]);
  }
  out(res);
  return 0;
}