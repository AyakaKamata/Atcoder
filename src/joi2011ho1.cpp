...
int main() {
  ...
  ll m, n; // 南北にMkm,東西にNkm
  in(m, n);
  ll k;
  in(k); // 調査対象領域の個数
  vector<string> NS(m);
  rep(i, 0, m) { // 北からi行目の区画の情報
    // 「ジャングル」は J，「海」は O，「氷」は I
    in(NS[i]);
  }
  vector<vector<pll>> Search(k);
  rep(i, 0, k) { // 調査対象領域
    ll a, b, c, d;
    in(a, b, c, d);
    Search[i] = {make_pair(a, b), make_pair(c, d)}; // a,b:北西の角,c,d:南東の角
  }
  // 2次元累積和の作成 (各地形ごとに)
  // 累積和配列はサイズ (m+1) x (n+1) として、1-indexedで計算する
  vector<vector<ll>> cJ(m + 1, vector<ll>(n + 1, 0));
  vector<vector<ll>> cO(m + 1, vector<ll>(n + 1, 0));
  vector<vector<ll>> cI(m + 1, vector<ll>(n + 1, 0));

  for (ll i = 1; i <= m; i++) {
    for (ll j = 1; j <= n; j++) {
      // NS のインデックスは 0-indexed なので i-1, j-1 を参照
      cJ[i][j] = cJ[i - 1][j] + cJ[i][j - 1] - cJ[i - 1][j - 1] +
                 (NS[i - 1][j - 1] == 'J');
  ...
  rep(i, 0, k) {
    ll a = Search[i][0].first, b = Search[i][0].second;
    ll c = Search[i][1].first, d = Search[i][1].second;
    ll jres = cJ[c][d] - cJ[a - 1][d] - cJ[c][b - 1] + cJ[a - 1][b - 1];
    ll ores = cO[c][d] - cO[a - 1][d] - cO[c][b - 1] + cO[a - 1][b - 1];
    ll ires = cI[c][d] - cI[a - 1][d] - cI[c][b - 1] + cI[a - 1][b - 1];
    out(jres, ores, ires);
  }
  return 0;
}