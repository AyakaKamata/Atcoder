...
int rec(int bit) {
  ...
  int &target = dp[bit];
  if (bit == (1 << M) - 1)
    return 0; // すべての種類を使い切ったら終了
  if (target != INF)
    return target; // メモ済みなら返す

  int done = 0;
  rep(i, 0, M) if (bit & (1 << i)) done += cnt[i];

  rep(nxt, 0, M) {
    if (bit & (1 << nxt))
      continue; // すでに使った種類はスキップ

    int nxt_cnt = rui[nxt][done + cnt[nxt] - 1];
    if (done > 0)
      nxt_cnt -= rui[nxt][done - 1];
    int cost = cnt[nxt] - nxt_cnt;

    chmin(target, rec(bit ^ (1 << nxt)) + cost);
  }
