...
int main() {
  ...
  vector<ld> xm(m), ym(m);
  rep(i, 0, m) { in(xm[i], ym[i]); }
  vector<vector<ld>> rhalf(m, vector<ld>(m, 0));
  rep(i, 0, m) rep(j, i + 1, m) {
    rhalf[i][j] = rhalf[j][i] =
        sqrt(twice(xm[i] - xm[j]) + twice(ym[i] - ym[j])) / 2;
  }
  onevecld rm(m);
  rep(i, 0, m) {
    ld tmp = INFL;
  ...
  rep(i, 0, m) {
    rep(j, 0, n) {
      chmin(rm[i], sqrt(twice(xm[i] - x[j]) + twice(ym[i] - y[j])) - r[j]);
    }
  }
  ld ans = INFL;
  rep(i, 0, m) { chmin(ans, rm[i]); }
  print(ans, 15);
  return 0;
}