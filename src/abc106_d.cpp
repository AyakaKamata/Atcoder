...
int main() {
  ...
  matrix cumsum_train = CumsumSquare(train);
  rep(i, 0, q) {
    ll p1, p2;
    in(p1, p2);
    ll ans = cumsum_train[p2][p2] - cumsum_train[p1 - 1][p2] -
             cumsum_train[p2][p1 - 1] + cumsum_train[p1 - 1][p1 - 1];
    out(ans);
  }
  return 0;
}