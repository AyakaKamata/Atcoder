...
#define fore(i, a) if (!(a).empty()) for (auto &i : a)
...
int dfs(int u, int p) {
    ...
    // sort pending chains in descending order
    sort(pending.begin(), pending.end(), greater<int>());

    bool used = false; // indicates whether u is used in a pairing that completes a path
    while(pending.size() >= 2){
        if(pending[0] + pending[1] + 1 >= K){
            // We can complete a path using these two chains with u in the middle.
    ...
    if(used){
        ...
        if(!pending.empty()){
            ret = pending[0] + 1; // extend the single pending chain upward via u.
        } else {
            ret = 1; // start a new pending chain from u.
        }
        if(ret == K){
            completeCount++;
            ret = 0;
...
int main(){
    ...
    int N;
    cin >> N >> K;
    int total = N * K; // total number of vertices
