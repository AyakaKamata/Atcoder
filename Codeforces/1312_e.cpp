...
ll rec(ll l, ll r) {
  ...
  // // 例: 区間を2つに分けて試す場合
  for (ll i = l; i < r; ++i) {
    chmin(target, rec(l, i) + rec(i + 1, r));
  }
  if (mer[l][r] != -1) {
    chmin(target, 1LL);
  }
...
int main() {
  ...
  dp.assign(n + 1, onevec(n + 1, INFL));
  mer.assign(n, onevec(n, -1));
  // Precompute which segments can be merged into a single value
  rep(len, 1, n + 1) {
    rep(l, 0, n - len + 1) {
      ll r = l + len - 1;
      if (len == 1) {
        ...
        rep(mid, l, r) {
          if (mer[l][mid] != -1 && mer[l][mid] == mer[mid + 1][r]) {
            mer[l][r] = mer[l][mid] + 1;
            break;
          }
        }
