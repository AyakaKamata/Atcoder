...
int main() {
  ...
  vector<pll> xy(4);
  rep(i, 0, 4) in(xy[i].first, xy[i].second);
  ll bac = crossp(xy, 1, 0, 2);
  ll bad = crossp(xy, 1, 0, 3);
  ll dca = crossp(xy, 3, 2, 0);
  ll dcb = crossp(xy, 3, 2, 1);

  if (bac == 0 && bad == 0 && dca == 0 && dcb == 0) {
    pll a = xy[0], b = xy[1], c = xy[2], d = xy[3];
    if (a > b)
      swap(a, b);
    if (c > d)
      swap(c, d);
